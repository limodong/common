...
function verifyPwdStrength(pwd) {
    if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#,.]).{6,12}$/.test(pwd)) {
        return "强";
    } else if (/^((?=.*[a-z])(?=.*[A-Z])(?=.*\d))|((?=.*[a-z])(?=.*[!@#,.])(?=.*\d))|((?=.*[A-Z])(?=.*[!@#,.])(?=.*\d)).{6,12}$/.test(pwd)) {
        return "中";
    } else if (/^((?=.*[a-z])(?=.*\d))|((?=.*[A-Z])(?=.*\d))|((?=.*[a-z])(?=.*[!@#,.]))|((?=.*\d)(?=.*[!@#,.])).{6,12}$/.test(pwd)) {
        return "低";
    }
    return "密码强度太低，无法通过校验";
}
console.log(`密码强度为：${verifyPwdStrength("123d@!asd")}`);